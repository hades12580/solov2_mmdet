...
Try to export mmdetection models

we do some experiments on SOLOV2
...
def main():
    ...
    if os.path.isdir(data_f):
        ...
        logging.info('Start to convert onnx...')
        f = 'solov2_r50_cityscapes.onnx'
        ...
        # prepare data
        data = dict(img=data_f)
        data = test_pipeline(data)
        data = scatter(collate([data], samples_per_gpu=1), [device])[0]

        print(data)

        img = torch.randn([1, 1280, 960, 3])
        # forward the model
        with torch.no_grad():
            result = model(return_loss=False, rescale=True, **data)
            torch.onnx.export(model, data, f, verbose=False, opset_version=11, input_names=['images'], keep_initializers_as_inputs=False,
                      output_names=['output'])
